...
{
    const AnimatedSprite* animatedSprite = sprite->animatedSprite;
    ...
    if (sprite->activeFrameTime > activeAnimation->frameTime[sprite->activeFrame]) {
        ...
        for (uint32 i = 0; i < nextAnimations.size; i++) {
            if (KeyCompare(sprite->activeAnimationKey, nextAnimations[i])) {
                break;
            }

            const uint32* exitToFrame = activeAnimation->frameExitTo[sprite->activeFrame].GetValue(nextAnimations[i]);
        ...
        if (!animTransition) {
            uint32 activeFrameNext = sprite->activeFrame + 1;
                        ...
                        VulkanSpritePipeline<S>* spritePipeline, AnimatedSprite* sprite)
{
    ...
    PsdFile psdFile;
    if (!LoadPsd(filePath.ToConstArray(), allocator, &psdFile)) {
        LOG_ERROR("Failed to open and parse level PSD file %.*s\n", filePath.size, filePath.data);
        return false;
    }

    sprite->animations.Initialize();
    ...
    while (true) {
        ...
        if (StringEquals(keyword, ToString("anim"))) {
            ...
            while (true) {
                uint32 nextLayerIndex = psdFile.layers.size;
                float64 earliestFrameStart = 1e6;
                for (uint32 i = 0; i < psdFile.layers.size; i++) {
                    ...
                    uint32 parentIndex = psdFile.layers[i].parentIndex;
                ...
                const PsdLayerInfo& layerInfo = psdFile.layers[nextLayerIndex];
                ImageData layerImageData;
                if (!LoadPsdLayerImageData(psdFile, nextLayerIndex, LayerChannelID::ALL, allocator, &layerImageData)) {
                    LOG_ERROR("Failed to load animation layer image data for %.*s, layer %.*s\n",
                              name.size, name.data, layerInfo.name.size, layerInfo.name.data);
                    return false;
                }

                VulkanImage layerImage;
                if (!LoadVulkanImage(device, physicalDevice, queue, commandPool,
                                     layerImageData.size.x, layerImageData.size.y, layerImageData.channels,
                                     layerImageData.data, &layerImage)) {
                    LOG_ERROR("Failed to Vulkan image for animation layer in %.*s, layer %.*s\n",
                              name.size, name.data, layerInfo.name.size, layerInfo.name.data);
                    return false;
                }

                if (!RegisterSprite(device, spritePipeline, layerImage, &currentAnim->frameSpriteIndices[frame])) {
                    LOG_ERROR("Failed to register sprite for %.*s, layer %.*s\n",
                              name.size, name.data, layerInfo.name.size, layerInfo.name.data);
                    return false;
                }
#if 0
                ...
                if (!psdFile.LoadLayerAtPsdSizeTextureGL(nextLayerIndex, LayerChannelID::ALL,
                                                         GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, allocator, &frameTextureGL)) {
                    LOG_ERROR("Failed to load animation texture GL for %s\n", name);
                    return false;
                }
                currentAnim->frameTextures[frame] = frameTextureGL;
#endif
                currentAnim->frameTime[frame] = layerInfo.timelineDuration;
            ...
            if (exitFromFrame == -1) {
                for (uint32 i = 0; i < currentAnim->numFrames; i++) {
    ...
    for (uint32 k = 0; k < sprite->animations.capacity; k++) {
        ...
        for (uint32 f = 0; f < anim->numFrames; f++) {
            const HashTable<uint32>* frameExitToTable = &anim->frameExitTo[f];
            for (uint32 j = 0; j < frameExitToTable->capacity; j++) {
                ...
                const uint32* exitToFrame = frameExitToTable->GetValue(*toAnimKey);
