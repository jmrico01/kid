...
const int16 PSD_COLOR_MODE_RGB = 3;
const uint32 STRING_MAX_SIZE = 1024;
...
internal int ReadUnicodeString(const_string str, FixedArray<char, S>* outString)
{
	...
	for (uint32 c = 0; c < outString->size; c++) {
...
struct PsdDescriptorItem
{
	...
	bool Load(const_string str, uint32* outParsedBytes);
};
...
bool PsdDescriptorItem::Load(const_string str, uint32* outParsedBytes)
{
	uint32 parsedBytes = 0;
                          ...
                          uint8 numChannels, int channelOffset, uint8* outData)
{
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			uint32 inIndex = y * width + x;
			uint32 invertedY = height - y - 1; // NOTE y-axis is inverted by this procedure
			uint32 outIndex = (invertedY * width + x) * numChannels + channelOffset;
...
internal bool ReadPackBitsData(const uint8* inData, Allocator* allocator, int width, int height,
                               uint8 numChannels, int channelOffset, uint8* outData)
{
	uint32 sizeRowLengths = height * sizeof(int16);
	uint16* layerRowLengths = (uint16*)allocator->Allocate(sizeRowLengths);
	if (!layerRowLengths) {
		LOG_ERROR("Not enough memory for layer row lengths, need %lu\n", sizeRowLengths);
		return false;
	}
	...
	for (int r = 0; r < height; r++) {
		...
		while (true) {
			...
			if (header == -128) {
				...
				int repeats = 1 - header;
				uint32 outIndex = (r * width + pixelX) * numChannels + channelOffset;
				for (int i = 0; i < repeats; i++) {
...
{
	const PsdLayerInfo& layerInfo = psdFile.layers[layerIndex];
	...
	uint32 sizeLayerData = layerWidth * layerHeight * numChannelsDest;
	...
	uint32 psdDataIndex = layerInfo.dataStart;
...
bool LoadPsd(const_string filePath, LinearAllocator* allocator, PsdFile* psdFile)
{
	...
	parsedBytes += 4;
	uint32 imageResourcesStart = parsedBytes;
	while (parsedBytes < imageResourcesStart + imageResourcesLength) {
		...
		uint32 resourceStart = parsedBytes;
		switch (resourceId) {
	...
	if (layerAndMaskInfoLength > 0) {
        ...
        psdFile->layers = allocator->NewArray<PsdLayerInfo>(layerCount);
		uint32 currentGroupEnd = psdFile->layers.size;

		for (uint32 layerIndex = 0; layerIndex < psdFile->layers.size; layerIndex++) {
			...
			parsedBytes += 4;
			uint32 extraDataStart = parsedBytes;
			...
			while (parsedBytes < extraDataStart + extraDataLength) {
				...
				parsedBytes += 4;
				// Ignored data here
				if (addLayerInfoLength % 2 == 1) {
					addLayerInfoLength++;
				}
				uint32 addLayerInfoStart = parsedBytes;
				...
				if (StringEquals(key, ToString("lyid"))) {
					...
					for (int32 i = 0; i < itemCount; i++) {
						...
						uint32 metadataStart = parsedBytes;
	...
	uint32 imageDataLength = psdFile.size - parsedBytes;
